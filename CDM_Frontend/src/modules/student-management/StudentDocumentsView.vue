...
<template>
  <section class="page-header">
    ...
    <p class="page-description">Read-only view of documents submitted for this student.</p>
  </section>
  ...
  <template v-else-if="student">
    ...
    <section v-if="!documents.length" class="panel state">
      <h2>No submitted documents</h2>
      <p>There are no student documents available in the registrar record yet.</p>
    </section>
    <section v-else class="documents">
      <article v-for="document in documents" :key="document.id" class="document-card">
        <div class="doc-head">
          <h2>{{ document.name }}</h2>
          <span class="badge">{{ title(document.status) }}</span>
        </div>
        <dl>
          <div>
          ...
          <div>
            <dt>Verified Date</dt>
            <dd>{{ date(document.verified_date) }}</dd>
          </div>
          <div>
            <dt>Verified By</dt>
