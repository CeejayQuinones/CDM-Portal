...
<template>
  <section class="admission-workflow"><header class="page-header"><p class="page-kicker">Admission</p><h1 class="page-title">{{ title }}</h1></header>
    ...
    <div class="placeholder-panel panel table-wrap" tabindex="0" role="region" :aria-label="title + ' table, scroll horizontally for more columns'"><table><thead><tr v-if="mode === 'applicants'"><th>Applicant</th><th>Cycle</th><th>Status</th><th>Created</th><th>Inspect</th></tr><tr v-else-if="mode === 'history'"><th>Result</th><th>Action</th><th>Version</th><th>Actor</th><th>Internal reason / decision</th><th>Date</th></tr><tr v-else><th>Select</th><th>Applicant / attempt</th><th>System score</th><th>Official result</th><th>Review</th></tr></thead><tbody>
      <template v-for="row in rows" :key="row.id">
        <tr v-if="mode === 'applicants'"><td>{{ row.name }}<p>{{ row.applicant_number }}</p></td><td>{{ row.cycle }}</td><td><span class="badge">{{ readable(row.status) }}</span></td><td>{{ new Date(row.created_at).toLocaleString() }}</td><td><button class="primary" :disabled="busy" @click="inspecting = row">Inspect applicant</button></td></tr>
        ...
        <tr v-else><td><input v-model="selected" type="checkbox" :value="row.id" :aria-label="'Select result '+row.id"></td><td>{{ row.name }}<p>{{ row.applicant_number }}</p><p>Attempt {{ row.attempt_number }} · Result {{ row.id }} · v{{ row.version }}</p></td><td>{{ row.system_percentage }}<details><summary>Topic evidence</summary><p v-for="(count,topic) in row.category_scores" :key="topic">{{ topic }}: {{ count }} / {{ row.category_maximums[topic] }}</p></details></td><td><span class="badge">{{ readable(row.official_status) }}</span> · {{ row.outcome }}<p>Score: {{ row.official_score ?? 'Not approved' }}</p><p v-if="row.registrar_pass">Registrar pass · {{ row.internal_reason }}</p></td><td><button class="primary" v-if="row.official_status === 'pending'" :disabled="busy" @click="begin('approve',row)">Approve</button><button class="primary" v-if="row.official_status === 'approved'" :disabled="busy" @click="begin('publish',row)">Publish</button><details v-if="row.official_status === 'published' || (row.attempt_number === 2 && !row.system_passed && !row.registrar_pass)"><summary>Other decisions</summary><button v-if="row.official_status === 'published'" :disabled="busy" @click="begin('correct',row)">Correct result</button><button v-if="row.attempt_number === 2 && !row.system_passed && !row.registrar_pass" :disabled="busy" @click="begin('override',row)">Registrar pass</button></details></td></tr>
    ...
    <details v-if="mode === 'history' && data?.events?.length" class="placeholder-panel panel"><summary>Recent Admission workflow events</summary><p v-for="event in data.events" :key="event.id">{{ event.created_at }} · {{ readable(event.action) }} · {{ event.subject_type }} {{ event.subject_id }}</p></details>
