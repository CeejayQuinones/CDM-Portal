...
<template>
<section class="admission-workflow"><p><router-link to="/admission">Admission home</router-link></p><h1>Result</h1>
<p v-if="loading" role="status">Loading result...</p><p v-if="error" role="alert">{{ error }}</p>
<div v-if="data?.published && !loading" class="panel"><span class="badge">Published result</span><h2 class="result-outcome">{{ data.result.outcome === 'PASSED' ? 'Passed' : 'Not Passed' }}</h2>
<dl class="summary-grid"><div><dt>Official score</dt><dd>{{ data.result.score === null ? 'Not disclosed' : data.result.score + ' / 100' }}</dd></div><div><dt>Attempt</dt><dd>{{ data.result.attempt_number }} of 2</dd></div><div><dt>Published</dt><dd>{{ new Date(data.result.published_at).toLocaleString() }}</dd></div></dl>
...
</div><div v-else-if="data && !loading" class="panel"><h2>Result not published yet</h2><p>Your result will appear here after your exam is reviewed and published by the Registrar.</p><router-link to="/admission">Check your next step on Admission home</router-link></div>
