...
const studentInitials = computed(() => studentFullName.value.split(' ').map((part) => part[0]).slice(0, 2).join('').toUpperCase() || 'ST')
const availableDocuments = computed(() => (selected.value?.student?.documents || [])
  .filter((document) => document.availability_status === 'available'))
const missingDocuments = computed(() => {
  ...
  if (!requestedRecord) {
    missing.push({
      ...
      document_type: { document_name: selected.value.document_type.document_name },
...
const requestedDocumentAvailable = computed(() => (selected.value?.student?.documents || []).some((document) =>
  document.document_type_id === selected.value?.document_type?.id
  && document.availability_status === 'available'))
const formatStatus = (value) => value ? value.replaceAll('_', ' ').replace(/\b\w/g, (letter) => letter.toUpperCase()) : 'Missing'
...
function updateQueueItem(updated) {
  const remainsInQueue = activeStatuses.includes(updated.status)
...
<template>
  ...
  <section class="dr-panel">
    <form class="toolbar" @submit.prevent="refresh(true)">
      ...
      <select v-model="status">
        ...
        <option v-for="value in activeStatuses" :key="value" :value="value">{{ value.replaceAll('_', ' ') }}</option>
      </select>
    ...
    <button v-for="item in requests" :key="item.id" class="queue-item" type="button" @click="selectRequest(item)">
      <span>
        ...
        <small>{{ item.student.student_number }} · {{ item.student.user.profile.first_name }} {{ item.student.user.profile.last_name }}</small>
      </span>
  ...
  <section v-if="selected" class="dr-panel">
    ...
    <p><strong>Student:</strong> {{ studentFullName }} ({{ selected.student.student_number }})</p>
    <p>
      <strong>Document:</strong> {{ selected.document_type.document_name }} · {{ selected.quantity }} copy/copies
    ...
    <section class="record-check" aria-labelledby="student-record-check-heading">
      <div class="record-check-heading">
        <div class="record-student-photo">
          <img v-if="studentProfile?.profile_photo" :src="studentProfile.profile_photo" :alt="`${studentFullName} photo`" />
        ...
        <div>
          ...
          <p>{{ selected.student.student_number }} · {{ selected.student.course?.course_code }} — {{ selected.student.course?.course_name }}</p>
          <p>Year {{ selected.student.year_level }} · {{ formatStatus(selected.student.student_status) }}</p>
        </div>
        <button type="button" class="record-link" @click="router.push({ name: 'student-details', params: { id: selected.student.id } })">View Student Record</button>
      </div>

      <p v-if="!requestedDocumentAvailable" class="record-warning">
        The requested document is not currently marked available in this student's document record. Registrar Staff may still continue processing the request.
      </p>
        ...
        <div>
          ...
          <ul v-else>
            <li v-for="document in availableDocuments" :key="document.id">
              <span>{{ document.document_type.document_name }}</span><strong class="available-label">Available</strong>
            </li>
          </ul>
        </div>
        <div>
          ...
          <ul v-else>
            <li v-for="document in missingDocuments" :key="document.id">
              <span>{{ document.document_type.document_name }}</span><strong class="missing-label">Missing</strong>
            </li>
          </ul>
        </div>
      </div>
    ...
    <label>Registrar remarks<textarea v-model="selected.remarks" rows="3"></textarea></label>
    <div class="actions">
      <button v-if="selected.status === 'pending'" @click="action('approve')">Approve & process</button>
      <button v-if="['pending', 'processing'].includes(selected.status)" class="danger" @click="action('reject')">Reject</button>
      ...
      <button v-if="activeStatuses.includes(selected.status)" class="secondary" @click="action('cancel')">Cancel</button>
    </div>
    ...
    <div v-for="appointment in selected.appointments" :key="appointment.id" class="appointment">
      <span>{{ appointment.appointment_date }} at {{ String(appointment.appointment_time).slice(0, 5) }} — {{ appointment.status }}</span>
    </div>
  </section>
</template>
