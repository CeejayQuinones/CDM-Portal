...
async function dispatch(method, rawUrl, body={}, config={}) {
  ...
  if(method==='get'&&path==='/registrar/appointment-availability/calendar'){const [year,month]=params.month.split('-').map(Number),days=new Date(year,month,0).getDate(),rows=[];for(let d=1;d<=days;d++){const date=`${params.month}-${String(d).padStart(2,'0')}`,override=await offlineDb.get('settings',`capacity-${date}`);rows.push({date,booked:data.appointments.filter(a=>a.appointment_date===date&&activeStatuses.includes(a.status)).length,capacity:override?.capacity||5})}return wrap({days:rows,settings:await offlineDb.get('settings','availability'),blocked_dates:await offlineDb.all('blocked_dates')})}
